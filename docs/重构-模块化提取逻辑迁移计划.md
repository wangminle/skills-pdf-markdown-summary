...
| **Phase 2** | 集成 Phase B/D 到主循环 + 验收机制 | 40% | ⏳ 待开始 |
| **Phase 3** | 智能方向判定 + 版式驱动 + Debug | 30% | ⏳ 待开始 |
...
### 5.3 Phase 2 详细任务 (中间 40% 工作量)

#### 5.3.1 集成 Phase B 对象对齐

- [ ] 完善 `refine_clip_by_objects()` 函数
- [ ] 在 `extract_figures.py` 中调用
- [ ] 在 `extract_tables.py` 中调用

#### 5.3.2 集成 Phase D Autocrop

- [ ] 集成 `detect_content_bbox_pixels()` 调用
- [ ] 实现 P0-1 远端边界单调性约束
- [ ] 集成 `trim_far_side_text_post_autocrop()` 调用
- [ ] 添加 autocrop 参数支持

#### 5.3.3 实现验收与回退机制

- [ ] 集成 `adaptive_acceptance_thresholds()` 调用
- [ ] 实现高度/面积/覆盖率检查
- [ ] 实现多级回退逻辑（refined → A-only → baseline）
- [ ] 添加验收失败日志记录

### 5.4 Phase 3 详细任务 (后 30% 工作量)
...
- [ ] 在主循环中调用 `select_best_caption()`
- [ ] 过滤引用类 caption

#### 5.4.3 版式驱动裁剪

- [ ] 迁移 `_adjust_clip_with_layout()` 函数
- [ ] 集成到 Phase A 后调用
- [ ] 集成到 autocrop 后调用

#### 5.4.4 Debug 可视化

- [ ] 记录各阶段边界框 (Baseline/Phase A/B/D)
- [ ] 集成 `save_debug_visualization()` 调用
- [ ] 生成 legend.txt 文件
...
| 文件 | 变更类型 | 说明 |
|-----|---------|------|
| `scripts/lib/refine.py` | 修改 | 添加文本裁切函数 |
| `scripts/lib/extract_figures.py` | 重写 | 完整实现主循环 |
| `scripts/lib/extract_tables.py` | 重写 | 完整实现主循环 |
| `scripts/lib/extract_helpers.py` | 修改 | 可能需要补充辅助函数 |
| `scripts/core/extract_pdf_assets.py` | 修改 | 可能需要调整参数传递 |
